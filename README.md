...
Замеры проводились на localhost (Windows, 12 threads), на моей машине, с собственным бенчмарк‑клиентом и redis-benchmark. Результаты ниже — локальные и зависят от железа, ОС и параметров запуска.
...
## Engineering Journey: Как я ускорил Go в 300 раз

Стартовая точка: ~27k RPS. Дальше — коротко по шагам и что именно дал прирост.
...
### 2) Снизил количество write‑syscalls
Я поменял стратегию flush:
- ответы буферизуются, не отправляются сразу
- `WSASend` вызывается только при заполнении буфера или когда входящий буфер пуст

### 2. Решение: Smart Batching & Syscall Reduction
Я переписал сетевой слой (handler.go), внедрив агрессивную буферизацию:
...
* Результат: Вместо 20,000 системных вызовов на батч я делаю 1-2 вызова.
...
2. Zero-Copy & Unsafe
Я агрессивно боролся с нагрузкой на Garbage Collector (GC), убирая лишние аллокации:
